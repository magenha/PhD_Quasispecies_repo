...
In order for the code to work properly, the following steps are needed:
	...
	4) Now we can run "script_findIDs.py":
		...
		>> Each node's ID is just t	he order (starting at 0) in which they appear in this file.
	...
	6) Within  ~/Data/, now we need to create a file called "metadata.csv":
		...
		>> It must contain three lines:
			...
			> Third, containing the length of the seq	h line of "nodesID.csv").
	...
	8) Now we can run the script "script_generateSubNetworks.py", that extracts the subnetwork corresponding to each variant:
		...
		>> This generates a folder ~/Data/GN/SubNetworks/ that contains one file per variant with the edges of the corresponding subnetwork.isted in the file "nodesID.csv".
		>> They appear in the order in which they were found.
	...
	6) Within  ~/Data/, now we need to create a file called "metadata.csv":
		>> There is an example of this file in each folder.
		>> It must contain three lThis folder contains code two reconstruct genotype-genotype networks, with two examples.
...
This folder contains code to reconstruct and visualize networks from genotype data, as well as a few network examples
that can be used to run some calculations. More scripts should be added in the future -- e.g. a script to collapse
genotype networks as parts of the sequence are ignored. 

Each script is ready to use except for the paths, which must be changed to provide the I/O locations for the data.
Additionally, each file expects to find some specific input ready. For example, the file that reconstructs the grand
network assumes that unique IDs have already been provided for each sequence. In the future, all these scripts will be
put together in a single file to run the whole pipeline smoothly. 

A series of examples of reconstructed networks can be found in the folder ~/NetExamples/. Two folders can be found
inside: ~/NetExamples/SubNetworks_nNodes-10/ and ~/NetExamples/SubNetworks_nNodes-100/. All examples provided are from
region 1 of the Qbeta phague. 	The first folder contains networks with connections between sequences of the 10 most
populated genotypes -- the second folder is the same for the top 100. From each file within these folders there is a
list of edges, so it should be possible to load them straightforwardly with any software for network analysis. For
example, it is possible to load the network in a file in python's networkx (nx) using three commands: 

	>> edges = np.loadtxt(fileName, delimiter=', ', dtype=int); 
	>> G = nx.Graph(); 
