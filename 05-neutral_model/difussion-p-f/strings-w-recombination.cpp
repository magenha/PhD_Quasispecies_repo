...
vector<char> recombine(vector<char> s1, vector<char> s2)
{
	int L = s1.size();
	int rec_point = L / 2;

	vector<char> recombined(L);
	for (int i = 0; i < rec_point; i++)
	{
		recombined[i] = s1[i];
		recombined[i + rec_point] = s2[i + rec_point];
	}
	return recombined;
}
...
int main(int argc, char *argv[])
{
	...
	fName
		<< "./dataOut_f_" << fstr << "_recombination.csv";
	...
	for (int t = 0; t < tMax; t++)
	{
		...
		// Add some replication to the process. This will triplicate the population
		int sizeofAuxX = aux_x.size();

		for (int j = 0; j < sizeofAuxX; j++)
		{
			vector<char> recombined_sequence;
			// Select two random integers
			int i_1, i_2;
			i_1 = random() % sizeofAuxX;
			i_2 = random() % sizeofAuxX;
			while (i_1 == i_2)
			{
				i_2 = random() % sizeofAuxX;
			}

			recombined_sequence = recombine(aux_x[i_1], aux_x[i_2]);
			aux_x.push_back(recombined_sequence);
			recombined_sequence = recombine(aux_x[i_2], aux_x[i_1]); // Now the reversed combination
			aux_x.push_back(recombined_sequence);
		}

		// Now select just n_rw random sequences within aux_x
		sizeofAuxX = aux_x.size();
