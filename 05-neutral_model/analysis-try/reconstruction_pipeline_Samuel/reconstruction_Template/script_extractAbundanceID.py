...
fIDName = os.path.join(dataPathIn, "nodesID.csv");

# Setting up folder to store abundances if it doesn't exist:
...
for thisFileName in allFileNames:
	...
	# File to save output:
	fOut = open(os.path.join(dataPathOut, thisFileName + ".csv"), 'w');
