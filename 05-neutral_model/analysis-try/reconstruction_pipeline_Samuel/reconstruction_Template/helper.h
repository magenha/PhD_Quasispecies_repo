	...
	*/ 
	...
	vector<int> rProtocol_nConsecutive(int nMiss, int seqLength, string fOutBaseName="./dummyOut"){
		...
		*/
		...
		// Files to store the renormalization protocol: 
	...
	vector<int> rProtocol_nRandom(int nMiss, int seqLength, string fOutBaseName="./dummyOut"){
		/* rProtocol_nRandom function: 

			This function produces a renormalization protocol that consists in ignoring nMiss non-consecutive
			nucleotides. 

			Inputs: 
				>> nMiss: Number of nucleotides that will be ignored. 
				>> seqLength: Length of the sequence. 
				>> fOutBaseName: To build the file where the renormalization protocol is stored. 
		...
		*/
		...
		for (int i=0; i<nMiss; i++){
			iRemove = rand()%(seqLength-i); 
			iErase.push_back(iKeep[iRemove]); 
			iKeep.erase(iKeep.begin()+iRemove); 
		}
		sort(iErase.begin(), iErase.end()); 


		for (int i=0; i<iKeep.size(); i++){
			fOutKeep << iKeep[i] << endl; 
		}
		for (int i=0; i<iErase.size(); i++){
			fOutErase << iErase[i] << endl; 
		}

		return iErase; 
	}

	vector<int> rProtocol_keepBetween(int iKeep0, int iKeepEnd, int seqLength, string fOutBaseName="./dummyOut"){
		/* rProtocol_keepBetween function: 

			This function produces a renormalization protocol that keeps every nucelotide between the two indicated (included)
		...
		*/
		...
		for (int i=0; i<iKeep0; i++){
			iErase.push_back(i); 
			fOutErase << i << endl; 
		}
		for (int i=iKeep0; i<=iKeepEnd; i++){
			fOutKeep << i << endl; 
		}
		for (int i=(iKeepEnd+1); i<seqLength; i++){
			iErase.push_back(i); 
			fOutErase << i << endl; 
		}
