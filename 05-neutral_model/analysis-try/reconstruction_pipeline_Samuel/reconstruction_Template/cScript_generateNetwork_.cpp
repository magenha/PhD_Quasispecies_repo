/* 
	...
	cScript_generateNetwork_.cpp: 
		...
		ACHTUNG!! 
			>> This is the implementation that indicates in which positions are located the mutations that link connected nodes. 
...
int main(int argc, char* argv[]){
	...
	int refID=0; 
	string netName="SARS-CoV-2_R1"; 
	ostringstream dataPathIn, dataPathOut, sysStr, fInName, fOutName;
	dataPathIn.str(""); 
	dataPathIn << "./Data/"; 
	gN::GNet gNet; 
	fInName.str(""); 
	fInName << dataPathIn.str() << "metadata.csv"; 
	...
	fInName.str(""); 
	fInName << dataPathIn.str() << "nodesID.csv"; 
	...
	cout << "Computing distro time: " << duration1.count() << endl; 
	...
	gNet.findConectionsAndWhereMutations(); 
	...
	fOutName.str(""); 
	fOutName << dataPathOut.str() << "whereMutations.csv"; 
	gNet.saveWhereMutations(fOutName.str()); 
	cout << "Grand Network saved! " << endl; 

	auto stop2 = high_resolution_clock::now(); 
	auto duration2 = duration_cast<seconds>(stop2 - stop1); 
	cout << "Computing connections time: " << duration2.count() << endl; 
