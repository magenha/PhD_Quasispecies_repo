...
mu_dict = {30: 271*10**-4, 43:271*10**-3}
WD = os.getcwd()
...
for t in temps:
    ...
    #Select the k most abundant at the beginning
    df_x = df_t.sort_values(by=L[0], ascending=False)
    k_list = list(df_x.index)[:k_most]
    #Select the k most abundant at the end
    df_x = df_t.sort_values(by=L[-1], ascending=False)
    k_list = k_list + [i for i in list(df_x.index)[:k_most]]
    k_list = list(set(k_list))

    #Create plot    
    plt.figure()
    for n,i in enumerate(k_list):
        ...
        plt.plot(x,y, 'o-', label=f"{n+1}:{i}")
        
    x = np.linspace(1,len(L), 100)
    y = np.exp(-1.0*np.e**(-1.0*mu_dict[t])*x) 
    plt.plot(x, y, '--', label=f'exponential mu*L={round(mu_dict[t],3)}')
    ...
    plt.savefig(f'./abundance_evolution_kmost_{t}.pdf', dpi=300, format='pdf')
    plt.savefig(f'./abundance_evolution_kmost_{t}.png', dpi=300, format='png')
