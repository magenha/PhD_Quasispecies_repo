...
for t in temps:
    ...
    for j, step in enumerate(L):
        ...
        populations.append(jaccknife_df(df['log'][''][t][step]))
    ...
    with open(f'./computes_results/th_{t}_{realization}_log.np', 'wb') as f:
        np.save(f, results_th)

    with open(f'./computes_results/phi_{t}_{realization}_log.np', 'wb') as f:
...
for q in list(quantities.keys()):
    for t in temps:  
        with open(f'./computes_results/{q}_{t}_{realization}_log.np', 'rb') as f:
...
for t in temps:
    ...
    plt.tight_layout()
    plt.title(f'phi')
plt.savefig(f'./pics_results/phi_log.jpg')
...
for t in temps:
    ...
    plt.tight_layout()
    plt.title(f'theta')
plt.savefig(f'./pics_results/th_log.jpg')
